...
res_maps = {
    ...
    "latent": {
        ...
        "x_key": "latent_factor",
        "csv_name": "ZEMA_HYD_BOTTLENECK_AUROC.csv",
...
groupby_keys = ["target_dim", "skip", x_key, "bae_type"]
...
figsize = (9, 3.5)
fig, axes = plt.subplots(1, 4, sharey=False, figsize=figsize)

for target_dim, ax in zip(auroc_res_mean["target_dim"].unique(), axes):
    subset_ssid = auroc_res_mean[auroc_res_mean["target_dim"] == target_dim]
    labels = []
...
if savefig:
    fig.savefig("zema-hyd-bottleneck-" + res_mode + ".png", dpi=500)
