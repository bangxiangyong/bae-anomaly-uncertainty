...
encoder_nodes = [input_dim*10,input_dim*3]
...
print("AUROC MEAN (OUTLIER) : {:.2f}".format(auroc_v1))
print("AUROC VAR (OUTLIER) : {:.2f}".format(auroc_v2))
...
ax1.boxplot([outp_inliers_train.mean(0).mean(-1),
             outp_inliers_valid.mean(0).mean(-1),
             np.concatenate((outp_inliers_valid.mean(0).mean(-1),outp_inliers_train.mean(0).mean(-1))),
             outp_outliers_test.mean(0).mean(-1),
             ])

ax2.boxplot([outp_inliers_train.var(0).mean(-1),
             outp_inliers_valid.var(0).mean(-1),
             np.concatenate((outp_inliers_valid.var(0).mean(-1),outp_inliers_train.var(0).mean(-1))),
             outp_outliers_test.var(0).mean(-1),
             ])
