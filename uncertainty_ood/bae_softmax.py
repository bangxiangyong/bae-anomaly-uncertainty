...
#==========EXPERIMENTAL (ADD SOFTMAX LAYER END)=========
...
class AnomalyLayer(torch.nn.Module):
    def __init__(self, inp_dim=2):
        ...
        self.pre_activation = torch.nn.LeakyReLU(0.01)
        # self.last_layer = torch.nn.Linear(inp_dim,1)
        self.last_layer = torch.nn.Linear(1, 1)
        self.sigmoid_layer = torch.nn.Sigmoid()

    def forward(self, x_rc_loss):
        x_out = self.pre_activation(x_rc_loss)
        while (len(x_out.shape) > 1):
            x_out = x_out.mean(-1)
        x_out = x_out.view(-1,1)
        x_out = self.last_layer(x_out)
        x_out = self.sigmoid_layer(x_out)

        return x_out


# pre_activation = torch.nn.LeakyReLU(0.01)
# last_layer = torch.nn.Linear(2,1)
# sigmoid_layer = torch.nn.Sigmoid()

x_inliers_train_tensor = torch.from_numpy(x_inliers_train).float()
x = x_inliers_train_tensor
y = x_inliers_train_tensor

y_out = bae_ensemble.autoencoder[0](x_inliers_train_tensor)
recon_loss = bae_ensemble.nll(autoencoder=bae_ensemble.autoencoder[0], x=x, y=y, mode="mu")
# recon_loss = recon_loss.view(-1,1)
# logit_last = sigmoid_layer(recon_loss)

#####========LAST LAYER=======

anomaly_layer = AnomalyLayer()
anomaly_loss = torch.nn.BCELoss()

target = torch.zeros(recon_loss.shape[0]).view(-1,1)

outp_logit = anomaly_layer(recon_loss)
anomaly_loss(outp_logit, target)

num_epoch = 1500
optim = torch.optim.Adam(anomaly_layer.parameters(), lr=0.01)
recon_loss = recon_loss.detach()
for epoch in range(num_epoch):
    outp_logit = anomaly_layer(recon_loss)
    loss = anomaly_loss(outp_logit, target)
...
#==================================
# grid_2d_tensor = torch.
# recon_loss_grid2d = bae_ensemble.nll(autoencoder=bae_ensemble.autoencoder[0], x=, y=y, mode="mu")

plt.figure()
plt.scatter(outp_logit[:,0].detach().numpy(), recon_loss.mean(-1))
