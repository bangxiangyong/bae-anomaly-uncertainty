...
#     half_iterations=half_iter, min_lr=min_lr, max_lr=0.01 if max_lr > 0.01 else max_lr
# )

# bae_model.init_scheduler(
#     half_iterations=len(train_loader) // 2, min_lr=1.99e-06, max_lr=0.008
# )


if isinstance(bae_model, BAE_SGHMC):
    bae_model.fit(train_loader, burn_epoch=2, sghmc_epoch=2)
    ...
    # time_method(bae_model.fit, train_loader, num_epochs=5)
    # time_method(bae_model.fit, train_loader, num_epochs=2)
...
y_labels = ood_loader.dataset.targets
outlier_pred = flatten_np(ae_outliers_pred[nll_key].mean(0)).mean(-1)

auroc_classes = []
for class_ in np.unique(y_labels):
    auroc_ood = calc_auroc(
        flatten_np(ae_inliers_pred[nll_key].mean(0)).mean(-1),
        outlier_pred[np.argwhere(y_labels == class_)[:, 0]],
    )
    auroc_classes.append(auroc_ood)
print(auroc_classes)
print(np.mean(auroc_classes))
...
train_data = next(iter(train_loader))[0][:2].detach().cpu().numpy()
in_data = next(iter(test_loader))[0][:2].detach().cpu().numpy()
out_data = next(iter(ood_loader))[0][:2].detach().cpu().numpy()

ae_train_sample = bae_model.predict(train_data, select_keys=[nll_key, "y_mu"])
...
# ae_train_sample = bae_model.predict(train_data, select_keys=["y_mu"])
# ae_inliers_sample = bae_model.predict(in_data, select_keys=["y_mu"])
# ae_outliers_sample = bae_model.predict(out_data, select_keys=["y_mu"])

crop_int = 2

fig, ((ax1, ax2, ax3), (ax4, ax5, ax6), (ax7, ax8, ax9)) = plt.subplots(3, 3)

ax1.imshow(np.moveaxis(train_data[0][:, crop_int:-crop_int, crop_int:-crop_int], 0, 2))
ax2.imshow(np.moveaxis(in_data[0][:, crop_int:-crop_int, crop_int:-crop_int], 0, 2))
ax3.imshow(np.moveaxis(out_data[0][:, crop_int:-crop_int, crop_int:-crop_int], 0, 2))

ax4.imshow(
    np.moveaxis(
        ae_train_sample["y_mu"].mean(0)[0],
        0,
        2,
    )
)
ax5.imshow(
    np.moveaxis(
        ae_inliers_sample["y_mu"].mean(0)[0],
        0,
        2,
    )
)
ax6.imshow(
    np.moveaxis(
        ae_outliers_sample["y_mu"].mean(0)[0],
        0,
        2,
    )
)
...
ax7.imshow(
    (
        ae_train_sample[nll_key]
        .mean(0)[0][:, crop_int:-crop_int, crop_int:-crop_int]
        .mean(0)
    )
)
ax8.imshow(
    (
        ae_inliers_sample[nll_key]
        .mean(0)[0][:, crop_int:-crop_int, crop_int:-crop_int]
        .mean(0)
    )
)
...
auroc_ood = calc_auroc(
    (
        flatten_np(
            ae_inliers_pred[nll_key].mean(0)[
                :, :, crop_int:-crop_int, crop_int:-crop_int
            ]
        )
    ).mean(-1),
    (
        flatten_np(
            ae_outliers_pred[nll_key].mean(0)[
                :, :, crop_int:-crop_int, crop_int:-crop_int
            ]
        )
    ).mean(-1),
...
# joe = flatten_np(
#     ae_inliers_pred[nll_key].mean(0)[:, :, crop_int:-crop_int, crop_int:-crop_int]
# )
#
# new_joe = []
# for joe_i in joe:
#     new_joe_i = np.sort(joe_i)[-int(0.1 * len(joe_i)) :]
#     new_joe.append(new_joe_i)
# new_joe = np.array(new_joe).mean(-1)
#
#


def get_top_n_nll(joe, top_perc=0.1):
    new_joe = []
    for joe_i in joe:
        new_joe_i = np.sort(joe_i)[-int(top_perc * len(joe_i)) :]
        new_joe.append(new_joe_i)
    new_joe = np.array(new_joe).mean(-1)
    return new_joe
...
top_perc = 0.01
top_inlier = get_top_n_nll(
    flatten_np(ae_inliers_pred[nll_key].mean(0).mean(1)), top_perc=top_perc
)
top_outlier = get_top_n_nll(
    flatten_np(ae_outliers_pred[nll_key].mean(0).mean(1)), top_perc=top_perc
)
auroc_top = calc_auroc(top_inlier, top_outlier)
print(auroc_top)
