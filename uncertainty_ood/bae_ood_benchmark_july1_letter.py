...
x_outliers_train, x_outliers_test = train_test_split(x_outliers, train_size = np.round(3/len(x_outliers),2),shuffle=True, random_state=random_seed)
...
# encoder_nodes = [input_dim*2,input_dim*4,input_dim*6]
encoder_nodes = [input_dim*4,input_dim*4,input_dim*4]
...
hard_preds_outlier_test_v1 = bae_predict_ood_v1(bae_ensemble, x_inliers_train, x_outliers_test, keys=["nll_mean", "nll_var"], perc_threshold = perc_threshold)
...
latent_outliers_test_mean = bae_pred_all(bae_ensemble, x_outliers_test, return_mean=False)["latent_mean"]
...
pyod_ori = evaluate_clf(clf, x_inliers_train, x_inliers_test, x_outliers_test)
...
#======================OC-NLL================
nll_inliers_train = np.concatenate((np.expand_dims(nll_inliers_train_mean,1),
                                    np.expand_dims(nll_inliers_train_var,1)),
                                   axis=1)

nll_inliers_test = np.concatenate((np.expand_dims(nll_inliers_test_mean,1),
                                   np.expand_dims(nll_inliers_test_var,1)),
                                  axis=1)

nll_outliers_test = np.concatenate((np.expand_dims(nll_outliers_test_mean,1),
                                   np.expand_dims(nll_outliers_test_var,1)),
                                  axis=1)
nll_outliers_train = np.concatenate((np.expand_dims(nll_outliers_train_mean,1),
                                   np.expand_dims(nll_outliers_train_var,1)),
                                  axis=1)

plt.figure()
plt.scatter(nll_inliers_train[:,0],nll_inliers_train[:,1])
plt.scatter(nll_inliers_test[:,0],nll_inliers_test[:,1])
plt.scatter(nll_outliers_test[:,0],nll_outliers_test[:,1])
plt.scatter(nll_outliers_train[:,0],nll_outliers_train[:,1])

pyod_mean_var_nll = evaluate_clf(clf, nll_inliers_train, nll_inliers_test, nll_outliers_test)
