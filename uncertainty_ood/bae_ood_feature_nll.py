...
# wavg
nll_feat_mean_id = cdf_test_id.mean(0)
nll_feat_var_id = 1 / np.clip(cdf_test_id.var(0), 0.00001, 1)

nll_feat_mean_ood = cdf_test_ood.mean(0)
nll_feat_var_ood = 1 / np.clip(cdf_test_ood.var(0), 0.00001, 1)

wavg_mean_id = (nll_feat_mean_id * nll_feat_var_id).sum(-1) / (nll_feat_var_id.sum(-1))
wavg_mean_ood = (nll_feat_mean_ood * nll_feat_var_ood).sum(-1) / (
    nll_feat_var_ood.sum(-1)
)
...
def convert_cdf_feature(nll_ref, nll_test, dist="norm", scaling_cdf=True):
    res = []
    for feat_i in range(nll_ref.shape[-1]):
        res_i = convert_cdf(
            nll_ref[:, 0, :, feat_i],
            nll_test[:, 0, :, feat_i],
            dist=dist,
...
nll_feat_test_id = convert_cdf_feature(
    nll_inliers_valid, nll_inliers_valid, dist="norm", scaling_cdf=True
)
nll_feat_test_ood = convert_cdf_feature(
    nll_inliers_valid, nll_outliers_test, dist="norm", scaling_cdf=True
)

nll_feat_mean_id = nll_feat_test_id.mean(0)
nll_feat_var_id = 1 / (1 + np.exp(nll_feat_test_id.var(0)))

nll_feat_mean_ood = nll_feat_test_ood.mean(0)
nll_feat_var_ood = 1 / (1 + np.exp(nll_feat_test_ood.var(0)))

wavg_id = (nll_feat_mean_id * nll_feat_var_id).sum(-1) / (nll_feat_var_id.sum(-1))
wavg_ood = (nll_feat_mean_ood * nll_feat_var_ood).sum(-1) / (nll_feat_var_ood.sum(-1))

print(calc_auroc(wavg_id, wavg_ood))
print(calc_auroc(nll_feat_mean_id.mean(-1), nll_feat_mean_ood.mean(-1)))


print(calc_auroc(nll_feat_mean_id.var(-1), nll_feat_mean_ood.var(-1)))

plt.figure()
plt.hist(nll_feat_mean_id.mean(-1), alpha=0.5, density=True)
plt.hist(nll_feat_mean_ood.mean(-1), alpha=0.5, density=True)

plt.figure()
plt.hist(wavg_id, alpha=0.5, density=True)
plt.hist(wavg_ood, alpha=0.5, density=True)

plt.figure()
plt.hist(nll_feat_mean_id.max(-1))
plt.hist(nll_feat_mean_ood.max(-1))
print(calc_auroc(nll_feat_mean_id.max(-1), nll_feat_mean_ood.max(-1)))

# plt.matshow(pd.DataFrame(nll_feat_mean_id).corr())
plt.figure()
sns.heatmap(
    pd.DataFrame(nll_feat_mean_id).corr(),
    annot=True,
)
plt.figure()
sns.heatmap(
    pd.DataFrame(nll_feat_mean_ood).corr(),
    annot=True,
)
...
print(len(np.argwhere(gmm_pred_tr_sq[:, i] >= 0.999)) / len(gmm_pred_tr_sq[:, i]))
print(len(np.argwhere(gmm_pred_tr_sq[:, i] >= 0.999)))
