...
def predict_ood_unc(
    ...
    prob_inliers_test, unc_inliers_test = convert_prob(
        convert_cdf(nll_ref, nll_inliers, dist=dist_cdf, scaling=scaling), *(None, None)
    )
    prob_outliers_test, unc_outliers_test = convert_prob(
        convert_cdf(nll_ref, nll_outliers, dist=dist_cdf, scaling=scaling),
        *(None, None)
    )
...
for dist_cdf in ["norm", "uniform", "expon", "ecdf"]:
    ...
    for unc_type in ["epistemic", "aleatoric", "total"]:
        ...
        exp_params_temp.update({"dist_cdf": dist_cdf, "unc_type": unc_type})
...
# plot_kde_auroc(
#     y_soft_pred[np.argwhere(y_true == 0)[:, 0]],
#     y_soft_pred[np.argwhere(y_true == 0)[:, 0]],
#     y_soft_pred[np.argwhere(y_true == 1)[:, 0]],
# )
...
print(calc_auroc(nll_inliers_test_mean, nll_outliers_test_mean))
print(calc_auroc(nll_inliers_test_var, nll_outliers_test_var))

nll_test_mean = nll_inliers_test.mean(-1)
nll_out_mean = nll_outliers_test.mean(-1)

print([calc_auroc(nll_test_mean[i, 0], nll_out_mean[i, 0]) for i in range(num_samples)])

joe_combined = [
    calc_auroc(nll_inliers_test.mean(0)[0][:, i], nll_outliers_test.mean(0)[0][:, i])
    for i in range(input_dim)
]
print(joe_combined)

joe = []
for bae_i in range(num_samples):
    joe_ = [
        calc_auroc(nll_inliers_test[bae_i][0][:, i], nll_outliers_test[bae_i][0][:, i])
        for i in range(input_dim)
    ]
    joe.append(joe_)
joe = np.array(joe)
...
print(np.argsort(joe.mean(0))[::-1])
print(joe.mean(0)[np.argsort(joe.mean(0))[::-1]])

# calc_auroc(nll_inliers_test.mean(0)[0][:, i], nll_outliers_test.mean(0)[0][:, i])

#
# np.argwhere(joe.mean(0) >= 0.85)
...
def evaluate_clf(clf, x_inliers_train, x_inliers_test, x_outliers, scaler=None):

    if scaler is not None:
        scaler_ = scaler()
        scaler_ = scaler_.fit(x_inliers_train)
        x_inliers_train = scaler_.transform(x_inliers_train)
        x_inliers_test = scaler_.transform(x_inliers_test)
        x_outliers = scaler_.transform(x_outliers)

    clf.fit(x_inliers_train)
    y_inliers_pred = clf.decision_function(x_inliers_test)
    y_outliers_pred = clf.decision_function(x_outliers)

    print(calc_auroc(y_inliers_pred, y_outliers_pred))

    return y_inliers_pred, y_outliers_pred


#
i = 0
latent_train = bae_ensemble.predict_latent_samples(x_inliers_train)[i]
latent_inliers = bae_ensemble.predict_latent_samples(x_inliers_test)[i]
latent_outliers = bae_ensemble.predict_latent_samples(x_outliers_test)[i]

# latent_train = bae_ensemble.predict_latent_samples(x_inliers_train).mean(0)
# latent_train = bae_ensemble.predict_latent_samples(x_inliers_valid)[0]
# latent_inliers = bae_ensemble.predict_latent_samples(x_inliers_test).mean(0)
# latent_outliers = bae_ensemble.predict_latent_samples(x_outliers_test).mean(0)

# latent_train = bae_ensemble.predict_latent_samples(x_inliers_train).var(0)
# # latent_train = bae_ensemble.predict_latent_samples(x_inliers_valid)[0]
# latent_inliers = bae_ensemble.predict_latent_samples(x_inliers_test).var(0)
# latent_outliers = bae_ensemble.predict_latent_samples(x_outliers_test).var(0)


# latent_scaler = StandardScaler
latent_scaler = None

in_, out_ = evaluate_clf(
    clf, latent_train, latent_inliers, latent_outliers, scaler=latent_scaler
)
...
auroc_list = []
threshes = []
for thres in np.sort(np.unique(nll_inliers_test_mean)):
    in_ = nll_inliers_test_mean[np.argwhere(nll_inliers_test_mean <= thres)[:, 0]]
    out_ = nll_inliers_test_mean[np.argwhere(nll_inliers_test_mean > thres)[:, 0]]
    if len(in_) >= min_samples and len(out_) >= min_samples:
        auroc_ = calc_auroc(in_, out_)
        auroc_list.append(auroc_)
        threshes.append(thres)

plt.figure()
plt.scatter(threshes, auroc_list)


plot_kde_auroc(
    nll_inliers_test_mean, nll_inliers_test_mean, nll_outliers_test_mean, mode="hist"
)

plot_kde_auroc(
    np.exp(nll_inliers_test_mean),
    np.exp(nll_inliers_test_mean),
    np.exp(nll_outliers_test_mean),
)


plt.figure()
plt.boxplot([nll_inliers_test_mean, nll_outliers_test_mean])
...
normalizer = unnormalised_likelihood.sum()
norm_ll = unnormalised_likelihood / normalizer
...
enc_architecture = ["x3", "x2", "x1", "d2"]
enc_architecture = ["x3", "x2", "x1"]
# enc_architecture = [
#     "x3",
#     "x2",
# ]
...
x = torch.from_numpy(x_outliers_train[:5]).float()

skip_outp = []
for enc_layer in encoder:
    if isinstance(enc_layer, DenseLayers):
        len_enc_layers = len(enc_layer.layers)
        for layer_i, dense_layer_i in enumerate(enc_layer.layers):
            x = dense_layer_i(x)
            if (
                isinstance(dense_layer_i, torch.nn.Linear)
                and layer_i < len_enc_layers - 3
            ):
                skip_outp.append(x)
...
# print("----DEC----")
ii = 0
len_skip_outp = len(skip_outp)
for dec_layer in decoder_mu:
    if isinstance(dec_layer, DenseLayers):
        for layer_i, dense_layer_i in enumerate(dec_layer.layers):
            x = dense_layer_i(x)
            if isinstance(dense_layer_i, torch.nn.Linear) and ii < len_skip_outp:
                print(x.shape)
                x += skip_outp[ii]
                ii += 1
    else:
...
nll_outliers_test_scaled = nll_scaler.transform(
    nll_outliers_test.mean(0)[0].mean(-1).reshape(-1, 1)
)


for k_components in range(2, 10):
...
# gm = GaussianMixture(n_components=best_k, random_state=rd_seed).fit(
#     np.concatenate(
#         (nll_inliers_valid_mean.reshape(-1, 1), nll_outliers_test_mean.reshape(-1, 1))
#     )
# )

print("means")
print(gm_squash.means_)
print("std")
print(np.sqrt(gm_squash.covariances_))


nll_min_ = np.min(nll_inliers_test_scaled)
nll_max_ = np.max(nll_outliers_test_scaled)
nll_x_axis = np.linspace(nll_min_, nll_max_, 100)
gmm_proba = gm_squash.predict_proba(nll_x_axis.reshape(-1, 1))
gmm_pdf = gm_squash.score_samples(nll_x_axis.reshape(-1, 1))


fig, ax = plt.subplots(1, 1)
ax2 = ax.twinx()
for k_ in range(gmm_proba.shape[-1]):
    ax.plot(nll_x_axis, gmm_proba[:, k_], label=str(k_))

# ax.plot(nll_x_axis, gmm_pdf)

# ax.plot(nll_x_axis, 4 * (gmm_inliers[:, 1] * (1 - gmm_inliers[:, 1])))
# ax.plot(nll_x_axis, gm.score_samples(nll_x_axis.reshape(-1, 1)))

ax2.hist(nll_inliers_valid_scaled, alpha=0.25, density=True)
ax2.hist(nll_inliers_test_scaled, alpha=0.25, density=True)
ax2.hist(nll_outliers_test_scaled, alpha=0.25, density=True)

# ax.plot(nll_x_axis, ECDF(nll_inliers_valid_mean)(nll_x_axis))

gmm_pred_tr_sq = gm_squash.predict_proba(nll_inliers_valid_scaled)
gmm_pred_in_sq = gm_squash.predict_proba(nll_inliers_test_scaled)
gmm_pred_out_sq = gm_squash.predict_proba(nll_outliers_test_scaled)


aurocs_sq = []

for i in range(best_k):
    auroc_ = calc_auroc(gmm_pred_in_sq[:, i], gmm_pred_out_sq[:, i])
    aurocs_sq.append(auroc_)
i = np.argmax(aurocs_sq)
plt.figure()
plt.boxplot([gmm_pred_tr_sq[:, i], gmm_pred_in_sq[:, i], gmm_pred_out_sq[:, i]])
print("AUROC SQ (BEST): {:.3f}".format(np.max(aurocs_sq)))

print(len(np.argwhere(gmm_pred_in_sq[:, i] >= 0.996)) / len(gmm_pred_in_sq[:, i]))

plt.figure()
plt.hist(gmm_pred_tr_sq[:, i], density=True, alpha=0.65, label="valid")
plt.hist(gmm_pred_in_sq[:, i], density=True, alpha=0.65, label="test-id")
plt.hist(gmm_pred_out_sq[:, i], density=True, alpha=0.65, label="out")
plt.legend()
...
i = 1
nll_scaler = RobustScaler()
nll_inliers_valid_scaled = nll_scaler.fit_transform(nll_inliers_valid.mean(0)[0])
nll_inliers_test_scaled = nll_scaler.transform(nll_inliers_test.mean(0)[0])
nll_outliers_test_scaled = nll_scaler.transform(nll_outliers_test.mean(0)[0])
...
plt.figure()
plt.plot(list(range(2, 10)), bics)
best_k_fw = np.argmin(bics) + 2


# FEATURE WISE GMM
gm_fw = GaussianMixture(n_components=best_k_fw, random_state=rd_seed).fit(
    nll_inliers_valid_scaled
)

gmm_pred_tr_fw = gm_fw.predict_proba(nll_inliers_valid_scaled)
gmm_pred_in_fw = gm_fw.predict_proba(nll_inliers_test_scaled)
gmm_pred_out_fw = gm_fw.predict_proba(nll_outliers_test_scaled)

plt.figure()
plt.boxplot([gmm_pred_tr_fw[:, i], gmm_pred_in_fw[:, i], gmm_pred_out_fw[:, i]])
print(
    "AUROC FW: {:.3f}".format(calc_auroc(gmm_pred_in_fw[:, i], gmm_pred_out_fw[:, i]))
)
...
# )
#
# gmm_pred_tr_sq = gm_squash.predict_proba(
#     nll_inliers_valid.mean(0)[0].mean(-1).reshape(-1, 1)
# )
# gmm_pred_in_sq = gm_squash.predict_proba(
#     nll_inliers_test.mean(0)[0].mean(-1).reshape(-1, 1)
# )
# gmm_pred_out_sq = gm_squash.predict_proba(
#     nll_outliers_test.mean(0)[0].mean(-1).reshape(-1, 1)
# )
#
# plt.figure()
# plt.boxplot([gmm_pred_tr_sq[:, i], gmm_pred_in_sq[:, i], gmm_pred_out_sq[:, i]])
# print(
#     "AUROC SQ: {:.3f}".format(calc_auroc(gmm_pred_in_sq[:, i], gmm_pred_out_sq[:, i]))
# )

# plt.figure()
# plt.boxplot(
#     [
#         nll_inliers_valid.mean(0)[0].mean(-1),
#         nll_inliers_test.mean(0)[0].mean(-1),
#         nll_outliers_test.mean(0)[0].mean(-1),
#     ]
# )

# plt.hist(gmm_pred_out[:, 1])
...
#
# list(bae_ensemble.optimisers[1].state_dict().keys())
# bae_ensemble.optimisers[0].state = collections.defaultdict(dict)
# bae_ensemble.optimisers[2].state = collections.defaultdict(dict)
...
# plt.figure()
# plt.boxplot([in_, out_])

# print(calc_auroc(in_, out_))

#
# nll_inlier_class = 1 - y_soft_pred[np.argwhere(y_true == 0)[:, 0]]
# nll_outlier_class = y_soft_pred[np.argwhere(y_true == 1)[:, 0]]
#
# nll_inlier_class = -np.log(nll_inlier_class)
# nll_outlier_class = -np.log(nll_outlier_class)
#
# # nll_inlier_class[np.isinf(nll_inlier_class)] = 0
# # nll_outlier_class[np.isinf(nll_outlier_class)] = 0
#
# nll_inlier_class = np.array([i for i in nll_inlier_class if not np.isinf(i)])
# nll_outlier_class = np.array([i for i in nll_outlier_class if not np.isinf(i)])
#
# nll_total = np.concatenate((nll_inlier_class, nll_outlier_class)).mean()
#
# print("NLL LOSS : " + str(nll_total))
...
# plt.figure()
# plt.scatter(nll_inliers_test_mean, nll_inliers_test_var)
# plt.scatter(nll_outliers_test_mean, nll_outliers_test_var)

# spearmanr(nll_inliers_test_mean, nll_inliers_test_var)[0]
# spearmanr(nll_outliers_test_mean, nll_outliers_test_var)[0]
