...
n_samples= 1500
X1 = np.random.normal(loc=0.2,scale=0.05,size=n_samples)

# X3 = np.random.normal(loc=0.8,scale=0.05,size=n_samples)

# X = np.expand_dims(np.concatenate((X1,X3)),1)
X = np.expand_dims(X1,1)

# plot True Gaussian
evaluate_range = np.linspace(-0.15, 1.15, 100)
pdf_X1 = norm.pdf(evaluate_range, loc=0.2, scale=0.05)
pdf_X3 = norm.pdf(evaluate_range, loc=0.8, scale=0.05)
...
#specify decoder-mu
decoder_mu = infer_decoder(encoder,activation="tanh",last_activation="sigmoid") #symmetrical to encoder
...
bae_ensemble = BAE_Ensemble(autoencoder=autoencoder,
                            anchored=True,
                            weight_decay=0.001,
                            num_samples=10, likelihood="gaussian", homoscedestic_mode="every")

# train mu network
bae_ensemble.fit(X,num_epochs=150)
...
# ax1.fill_between(evaluate_range, y_nll_samples_mean+2*y_nll_samples_std, y_nll_samples_mean-2*y_nll_samples_std, alpha=0.5)
ax1.fill_between(evaluate_range, y_nll_ub, y_nll_lb, alpha=0.5)

ax1.plot(evaluate_range,true_pdf, color="tab:orange")
ax1.scatter(X,X*0, c="blue")
