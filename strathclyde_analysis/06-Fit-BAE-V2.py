...
target_dims_all = [2]
# forging_sensors_all = [[71,64]]

for target_dim in target_dims_all:
    for sensor_i in heating_sensors_all if mode == "heating" else forging_sensors_all:
    # for sensor_i in forging_sensors_all:
        for apply_fft in [True, False]:
        # for apply_fft in [False]:
            exp_params = {
                "apply_fft": apply_fft,
                "mode": mode,
                "ss_id": sensor_i,
            ...
            x_heating = [
                heating_trace[:, heating_sensors] for heating_trace in heating_traces
            ]
            x_forging = [
                forging_trace[:, forging_sensors] for forging_trace in forging_traces
            ]
            ...
            print(
                "AUROC VAR (TRAIN-TEST):" + str(calc_auroc(var_nll_train, var_nll_id))
            )
            ...
            # exp_man.update_csv(exp_params=res, csv_name="STRATH_BAE_TEST_HEAT_ALLDIM.csv")
