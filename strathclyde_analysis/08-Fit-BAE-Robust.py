...
mode = "forging"
# mode = "heating"
resample_factor = 1 if mode == "forging" else 10
pickle_path = "pickles"
# apply_fft = False
# apply_fft = True

tukey_threshold = 1.00
...
# === ITERATE ===
for random_seed in [55, 100, 33, 99, 321, 3, 111, 88, 9158, 160]:
    bae_set_seed(random_seed)
    for target_dim in np.unique(filtered_res["target_dim"]):
        sensors = filtered_res[filtered_res["target_dim"] == target_dim]["ss_id"].values

        for sensor_i in [9]:
            # for sensor_i in forging_sensors_all:
            # for apply_fft in [True, False]:
            for apply_fft in [True]:
                exp_params = {
                    "apply_fft": apply_fft,
                    "mode": mode,
                    "ss_id": sensor_i,
                    "ss_name": column_names[sensor_i],
                ...
                heating_sensors = np.array([sensor_i])
                # heating_sensors = np.array([75])
                # heating_sensors = np.array([75])
                ...
                x_heating = [
                    heating_trace[:, heating_sensors]
                    for heating_trace in heating_traces
                ]
                x_forging = [
                    forging_trace[:, forging_sensors]
                    for forging_trace in forging_traces
                ]
                ...
                if resample_factor > 1:
                    x_id_train = Resample_Sensor().transform(
                        x_id_train, n=resample_factor
                    )
                    x_id_test = Resample_Sensor().transform(
                        x_id_test, n=resample_factor
                    )
                    x_ood_test = Resample_Sensor().transform(
                        x_ood_test, n=resample_factor
                    )
                ...
                print(
                    "AUROC NLL (TRAIN-TEST):" + str(calc_auroc(e_nll_train, e_nll_id))
                )
                print("AUROC NLL (TEST-OOD):" + str(calc_auroc(e_nll_id, e_nll_ood)))

                print(
                    "AUROC VAR (TRAIN-TEST):"
                    + str(calc_auroc(var_nll_train, var_nll_id))
                )
                print(
                    "AUROC VAR (TEST-OOD):" + str(calc_auroc(var_nll_id, var_nll_ood))
                )
                        ...
                        [
                            {
                                v: k
                                for k, v in zip(
                                    tukey_flags.sum(0), np.arange(tukey_flags.shape[-1])
                                )
                            }
                        ]
                    ).T
                ...
                exp_man.update_csv(
                    exp_params=res, csv_name="STRATH_BAE_TEST_" + mode + "_ROBUST.csv"
                )
